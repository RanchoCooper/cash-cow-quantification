...
import (
	...
	http2 "quants/adapter/dependency/http"
	"quants/domain/repo"
	...
	for {
		...
		for _, coinType := range coinList {
			...
			if c.Backtest {
				...
				result := http2.BinanceClinet.GetTickerKLine(ctx, coinType, c.GetInterval(), 1, c.GetStartTime(), c.GetEndTime())
				marketPrice = ((*result)[0].High - (*result)[0].Low) / 2
				c.UpdateStartTime()
			} else {
				marketPrice = http2.BinanceClinet.GetTickerPrice(ctx, coinType).Price
			}
			...
			if gridBuyPrice >= marketPrice {
				...
				if ok {
					// 买入成功
					http2.DingDingClient.SendDingDingMessage(fmt.Sprintf("买入成功。币种: %s, 数量: %f, 价格: %f", coinType, gridBuyQuantity, gridBuyPrice), false)
					c.SetRatio(coinType)
					c.SetRecordPrice(coinType, gridBuyPrice)
					c.ModifyPrice(coinType, marketPrice, step+1, marketPrice)
				} else {
					// 买入失败
					http2.DingDingClient.SendDingDingMessage(fmt.Sprintf("买入失败。币种: %s, 数量: %f, 价格: %f", coinType, gridBuyQuantity, gridBuyPrice), false)
				}
				...
				if step == 0 {
					...
					if ok {
						// 卖出成功
						http2.DingDingClient.SendDingDingMessage(fmt.Sprintf("卖出成功。币种: %s, 数量: %f, 价格: %f, 预计盈利: %f", coinType, gridSellQuantity, gridSellPrice, profitUSDT), false)
						c.SetRatio(coinType)
						c.ModifyPrice(coinType, marketPrice, step-1, marketPrice)
						c.RemoveRecordPrice(coinType)
					} else {
						// 卖出失败
						http2.DingDingClient.SendDingDingMessage(fmt.Sprintf("卖出失败。币种: %s, 数量: %f, 价格: %f", coinType, gridSellQuantity, gridSellPrice), false)
					}
				}
			} else {
